...
class Apk_parser{
    ...
    public function getXML($node=NULL, $lv=-1)
    {
        ...
        $xml = isset($node['line']) && ($node['line'] == 0 || $node['line'] == $this->line) ? '' : "\n".str_repeat('  ', $lv);
        $xml .= $node['tag'];
        isset($node['line']) && $this->line = $node['line'];
    ...
    public function getMinSdkVersionName()
    {
        $names = array(
            1   => '1.0',
            2   => '1.1',
        ...
        $code = $this->getMinSdkVersion();
        $name = 'Android';
        if (isset($names[$code])) {
            $name = $name.' '.$names[$code];
        }
        return $name;
    }
    ...
    public function getPermission()
    {
        $act = array();
        for ($id=0; true; $id++){
            $ret = $this->getAttribute("manifest/uses-permission[{$id}]", 'android:name');
            if (!$ret) break;
            $act[] = $ret;
        }
        return implode(';', $act);
